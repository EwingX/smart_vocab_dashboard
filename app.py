...
USERS = [
    {"id": "user1", "name": "予呈99"},
    {"id": "user2", "name": "我要洋人死"},
    {"id": "user3", "name": "aaa单词大户"},
]

SEED_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "words.json")
...
- part_of_speech: 该单词最常用的词性缩写（如 n., v., adj., adv., prep., conj. 等）
- en_definition: 简明易懂的纯英文释义，尽量控制在一句话内，类似牛津词典的简明版
- definition: 精炼中文释义，控制在20字以内（不含词性前缀）
...
def get_words_file():
    ...
    return os.path.join(data_dir, f"words_{user}.json")


def load_words():
    words_file = get_words_file()
    if not os.path.exists(words_file) and os.path.exists(SEED_FILE):
        import shutil
        shutil.copy2(SEED_FILE, words_file)
    try:
    ...
    for w in data["words"]:
        ...
        if "type" not in w:
            ...
            changed = True
        if "part_of_speech" not in w:
            m = re.match(r'^([a-z]+\.)\s*(.*)', w.get("definition", ""))
            if m:
                w["part_of_speech"] = m.group(1)
                w["definition"] = m.group(2)
            else:
                w["part_of_speech"] = ""
            changed = True
        if "en_definition" not in w:
            w["en_definition"] = ""
            changed = True

    if changed:
        _write_words(data, words_file)

    return data


def _write_words(data, words_file=None):
...
def call_deepseek(word):
    ...
    parsed.setdefault("en_definition", "")
...
def require_login():
    ...
    if "user" not in session and request.path not in ("/select", "/logout"):
        if request.path.startswith("/api/"):
            return jsonify({"error": "未选择用户"}), 401
    ...
    if request.method == "POST":
        ...
        for u in USERS:
            if u["id"] == user_id:
                ...
                return redirect(url_for("index"))
        return render_template("select.html", users=USERS, error="请选择一个用户")
    return render_template("select.html", users=USERS, error="")
...
def api_enrich_word(word_id):
    with file_lock:
        ...
        for key in ("word", "phonetic", "definition", "sentence", "translation",
                     "variations", "phrases", "part_of_speech", "en_definition"):
            if key in enriched:
...
if __name__ == "__main__":
    ...
    print("=" * 56)
    print("  Sync_Vocab  /  多用户智能单词本")
    print("=" * 56)
    print(f"  Data dir  : {os.environ.get('DATA_DIR', os.path.dirname(os.path.abspath(__file__)))}")
